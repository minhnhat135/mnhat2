...
# --- HELPER FUNCTIONS (COPIED FROM main.py FOR INDEPENDENCE) ---
...
def random_user_agent():
    ...
    # Ensure a realistic Windows NT version for the Chrome version
    win_nt_version = "10.0"

    return (
        f"Mozilla/5.0 (Windows NT {win_nt_version}; Win64; x64) "
...
def check_card_gate12(session, line, cc, mes, ano, cvv, bin_info, cancellation_event, get_gate12_mode, _get_charge_value, custom_charge_amount=None):
    """
    Logic for Gate 12: Charge 0.5$ V12 or Live Check.
    Uses random data similar to Gate 1.
    """
    ...
    try:
        ...
        # --- Step 1: Tokenize card with random formId ---
        tokenize_url = "https://pay.datatrans.com/upp/payment/SecureFields/paymentField"
        
        # Generate random 12-digit number for formId
        ...
        base_payload = {
            "account_uuid": "28846c84-f223-4e0f-98d3-1bbb2759ca18",
            "test_mode": False,
            ...
            "raisenow_parameters": {
                ...
                "solution": {
                    "uuid": "26acf3f7-5be6-48bd-962e-f50a5de77eff",
                    "name": "Berlin Marathon - Runner",
                    "type": "donate"
                },
                "product": {
                    "name": "tamaro",
                    "source_url": "https://donate.raisenow.io/qsjvv?lng=en",
                    "uuid": "self-service",
                    "version": "2.16.0"
                }
            },
            ...
            "payment_information": {
                "brand_code": "amx", # Note: 'amx' might need to be dynamic based on bin_info if available
                "cardholder": cardholder,
                "expiry_month": mes.zfill(2),
                "expiry_year": ano,
                "transaction_id": transaction_id
            },
            "profile": "8bb14581-041e-4625-b9a9-4a9a2c283515",
            "return_url": "https://donate.raisenow.io/qsjvv?lng=en&rnw-view=payment_result"
        }

        # --- CHARGE MODE ---
        if gate12_mode == 'charge':
            charge_value = _get_charge_value('12', custom_charge_amount)
        ...
        # --- LIVE CHECK MODE ---
            ...
            payment_payload = base_payload.copy()
            # Live mode needs an amount in the payload for validation, but it won't be charged
        ...
        logger.error(f"Unknown error in Gate 12 for line '{line}': {e}", exc_info=True)
        return 'error', line, f"Gate 12 System Error: {e}", bin_info
